 ...
 *
 * am::Switch and am::ProbabilitySwitch are am::ActivityGraphItems, but
 * are not translated to CallSequenceItems. Instead they end the current
 * CallSequence and are added as its sibling. When the am::Switch or the
...
void Converter::work( const am::ModeLabelAccess_ptr& am, am::ModeLabelAccess* ) {
	if ( _mode == PreOrder ) {
		if ( auto&& enumMode = ecore::as<am::EnumMode>( am->getData()->getMode() ) ) {
			/* A ModeLabelAccess for an EnumMode is translated to a SwitchPoint. */
...
void Converter::workSwitch( const ecore::Ptr<C>& am ) {
	if ( _mode == PreOrder ) {
		...
		/* The end of this am::Switch, any am::SwitchEntry and an
		 * am::SwitchDefault will lead to the creation of a new
		 * CallSequence. */
		_callSequence = sm3::CallSequence_ptr();
		...
		/* A new CallSequence starts after the Switch. It replaces the
		 * previous one. */
		_callSequence = sm3::create<sm3::CallSequence>();
		setName( *_callSequence );
		auto thisSwitch = _oc.make<sm3::ModelFactory, sm3::Switch>( am );
		details::addAsSibling( thisSwitch, _callSequence );
		_graphEntries.pop_back();
		_graphEntries.push_back( _callSequence );
