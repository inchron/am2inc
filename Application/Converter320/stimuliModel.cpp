 ...
 */
void Converter::work( const am::InterProcessStimulus_ptr& am,
					  am::InterProcessStimulus* ) {
	if ( _mode == PreOrder ) {
...
void Converter::work( const am::PeriodicBurstStimulus_ptr& am,
					  am::PeriodicBurstStimulus* ) {
	if ( _mode == PreOrder ) {
		auto gen = _oc.make<sm3s::StimulationFactory,
							StimulusTrait<am::PeriodicBurstStimulus>::type>( am );
		...
		{ /* Inherited from am::FixedPeriodic */
			gen->setPeriod(
				AttributeCreator<sm3::Time, am::ModelPackage>()( am->getRecurrence() ) );
			auto startOffset = gen->getStartOffsetVariation();
			startOffset->setType( sm3s::StartOffsetVariationType::Min );
			startOffset->setMin(
				AttributeCreator<sm3::Time, am::ModelPackage>()( am->getOffset() ) );
			startOffset->setMax(
				AttributeCreator<sm3::Time, am::ModelPackage>()( am->getOffset() ) );
		}
		...
		auto burstLength =
			AttributeCreator<sm3::Time, am::ModelPackage>()( am->getBurstLength() );
		auto interArrival = AttributeCreator<sm3::Time, am::ModelPackage>()(
			am->getOccurrenceMinDistance() );
...
void Converter::work( const am::PeriodicStimulus_ptr& am, am::PeriodicStimulus* ) {
	if ( _mode == PreOrder ) {
		auto gen =
			_oc.make<sm3s::StimulationFactory, StimulusTrait<am::PeriodicStimulus>::type>(
				am );
		const auto& name = am->getName();
		...
		gen->setMinimumInterArrivalTime(
			AttributeCreator<sm3::Time, am::ModelPackage>()( am->getMinDistance() ) );

		if ( am::ITimeDeviation_ptr jitter = am->getJitter() ) {
			auto min = [&jitter] {
				return AttributeCreator<sm3::Time, am::ModelPackage>()(
					jitter->getLowerBound() );
			};
			auto max = [&jitter] {
				return AttributeCreator<sm3::Time, am::ModelPackage>()(
					jitter->getUpperBound() );
			};
			...
			if ( jitter->eClass() == amPkg->getTimeConstant() ) {
				...
				variation->setSigma( AttributeCreator<sm3::Time, am::ModelPackage>()(
...
void Converter::work( const am::PeriodicSyntheticStimulus_ptr& am,
					  am::PeriodicSyntheticStimulus* ) {
	if ( _mode == PreOrder ) {
		...
		ecore::as<sm3s::StimulationScenario>( _model->getDefaultScenario() )
			->getGenerators()
			.push_back_unsafe( periodic );

		auto gen = _oc.make<sm3s::StimulationFactory,
							StimulusTrait<am::PeriodicSyntheticStimulus>::type>( am );
		gen->setName( am->getName() );
		gen->setClock( _idealClock );
		...
		for ( auto&& occurrence : am->getOccurrenceTimes() )
			gen->getEntries().push_back_unsafe(
				AttributeCreator<sm3::Time, am::ModelPackage>()( occurrence ) );
 ...
 */
void Converter::work( const am::RelativePeriodicStimulus_ptr& am,
					  am::RelativePeriodicStimulus* ) {
	if ( _mode == PreOrder ) {
		auto gen = _oc.make<sm3s::StimulationFactory,
							StimulusTrait<am::RelativePeriodicStimulus>::type>( am );
		const auto& name = am->getName();
		...
		if ( am::ITimeDeviation_ptr jitter = am->getNextOccurrence() ) {
			...
			if ( jitter->eClass() == amPkg->getTimeGaussDistribution() ) {
				...
				gen->setPeriod(
					AttributeCreator<sm3::Time, am::ModelPackage>()( tgd->getMean() ) );
				auto lower = AttributeCreator<sm3::Time, am::ModelPackage>()(
					jitter->getLowerBound() );
				auto upper = AttributeCreator<sm3::Time, am::ModelPackage>()(
					jitter->getUpperBound() );
				variation->setMin( lower );
				variation->setMax( upper );
				variation->setType( sm3::PeriodVariationType::Normal );
				variation->setSigma(
					AttributeCreator<sm3::Time, am::ModelPackage>()( tgd->getSd() ) );
...
void Converter::work( const am::SingleStimulus_ptr& am, am::SingleStimulus* ) {
	if ( _mode == PreOrder ) {
		auto gen =
			_oc.make<sm3s::StimulationFactory, StimulusTrait<am::SingleStimulus>::type>(
				am );
		gen->setName( am->getName() );
		gen->setClock( _idealClock );

		gen->getEntries().push_back_unsafe(
			AttributeCreator<sm3::Time, am::ModelPackage>()( am->getOccurrence() ) );
		ecore::as<sm3s::StimulationScenario>( _model->getDefaultScenario() )
			->getGenerators()
			.push_back_unsafe( gen );
	}
