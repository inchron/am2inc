...
void Converter::work( const am::Task_ptr& am, am::Task* ) {
	if ( _mode == PreOrder ) {
		...
		static const ecore::EString callbackServer( "callbackServer" );
		for ( const auto& property : am->getCustomProperties() ) {
			if ( equals( property->getKey(), callbackServer ) ) {
				if ( ResolveValue::resolve<ecore::EBoolean>( property->getValue() ) ) {
					process->setServer( true );
					// process->setExtendedTask( true );
					// process->setActiveAtBoot( true );
				}
				break;
			}
		}
...
/** An am::ConditionDisjunction is aggregated by an am::SwitchEntry::condition,
...
void Converter::workConditionDisjunction( const ecore::Ptr<C>& am ) {
	if ( _mode == PreOrder ) {
		...
		/* 1..* ConditionDisjunctionEntry interface -> ModeCondition interface ->
		 * ModeValueCondition
		 * ModeLabelCondition */
		for ( auto&& entry : am->getEntries() ) {
			if ( auto amModeCondition = ecore::as<am::ModeCondition>( entry ) ) {
				...
				condition->getConjunctions().push_back_unsafe( conjunction );
							...
							ecore::as<am::ConditionConjunction>( entry ) ) {
